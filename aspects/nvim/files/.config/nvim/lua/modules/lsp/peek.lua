-- Copied from the nvim-treesiter-textobjects plugin
-- This module was removed in https://github.com/nvim-treesitter/nvim-treesitter-textobjects/pull/661
...
function M.is_new_signature_handler()
...
function M.make_preview_location_callback(query_string, query_group, context)
  ...
  local callback = function(err, method, result)
    ...
    local _, textobject_at_definition =
      shared.textobject_at_point(query_string, query_group, buf, { range.start.line + 1, range.start.character })
  ...
  if M.is_new_signature_handler() then
